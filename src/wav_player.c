...
static void _callback_new(sound_cli_s * wpc, wav_player_playback_completed_cb cb, void *user_data)
{
	sound_callback_cb_info_s *cb_info;
	sound_msg_buff_s *buff;
	int i;
...
int wav_player_start(const char *path, sound_type_e type, wav_player_playback_completed_cb cb, void *user_data,  int * id)
{
	...
	/* Send msg */
	send_msg = muse_core_msg_json_factory_new(api, MUSE_TYPE_INT, "module", MUSE_SOUND,
												MUSE_TYPE_INT, PARAM_MODE, MODE_WAV,
												MUSE_TYPE_STRING, PARAM_PATH, path, 0);
	if (send_msg) {
...
int wav_player_stop(int id)
{
	...
	/* Send msg */
	send_msg = muse_core_msg_json_factory_new(api, MUSE_TYPE_INT, "module", MUSE_SOUND, 0);
