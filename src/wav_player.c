...
static int _parse_stream_info(sound_stream_info_h stream_info, char** stream_type, int *stream_id)
{
	...
	LOGI("parsed stream_type = [%s], stream_id = [%d]", stream_type, stream_id);
...
int wav_player_stop(int id)
{
	...
	if (wpc == NULL)
		RET_ERROR(WAV_PLAYER_ERROR_INVALID_OPERATION, "failed to get wpc!!!! list=%p[%d], id=%d", handle_mgr.handles, handle_mgr.handle_count, id);
