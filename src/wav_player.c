...
static int _client_wait_for_cb_return(muse_sound_api_e api, sound_callback_cb_info_s *cb_info, char **ret_buf, int time_out)
{
	...
	if (msg) {
		*ret_buf = msg->msg;
		...
		muse_core_msg_json_deserialize(PARAM_RET, *ret_buf, NULL, &ret, NULL, MUSE_TYPE_ANY);
	} else {
	...
	while (g_atomic_int_get(&cb_info->running)) {
		...
		do {
			...
			while (offset < len) {
				...
				if (muse_core_msg_json_deserialize(PARAM_API, recvMsg + offset, &len, &api, &err, MUSE_TYPE_INT)) {
